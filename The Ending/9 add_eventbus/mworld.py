...
class Organism(object):
    ...
    def reproduce(self)->  "None | Organism" :
        ...
        child.event_bus.publish("organisms_born",child)
        return child
...
#记录存活时间
    ...
    def death(self) -> None:
        ...
        self.event_bus.publish("organisms_died",self)
...
#先算步数
    ...
    def eat(self) -> None:
        ...
        if (self.life_x, self.life_y) in self.prey:  # 在猎物里面
            ...
            self.event_bus.publish("organisms_ate", self)  # 来自事件总线的提示
...
def main() -> None:
    ...
    #订阅生物死亡事件
    def on_organisms_died(org) -> None:
        print(f"玩家{org}已经死亡")
    event_bus.subscribe("organisms_died",on_organisms_died)

    #订阅生物进食事件
    def on_organisms_ate(org) -> None:
        print(f"{org}吃了一个食物，开心心")
    event_bus.subscribe("organisms_ate",on_organisms_ate)

    #订阅生物出生事件
    def on_organisms_born(org) -> None:
        print(f"新生物: {org} 出生了,世代 {org.generation}")
    event_bus.subscribe("organisms_born",on_organisms_born)
    ...
    world1=World()
    for i in range(WorldConfig().number_of_organisms):
        tjm=Organism(world1,event_bus=event_bus)
        world1.add_organisms(tjm)
